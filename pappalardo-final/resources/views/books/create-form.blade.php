...
?>
<x-layout-main>
    <x-slot:title>Agregar un Libro</x-slot:title>

    <section>
        <h1 class="mb-3">Agregar un Libro</h1>

        <form action="{{  route('books.create.process') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="pages" name="pages" class="form-control" @error('pages') aria-describedby="error-pages" @enderror value="{{ old('pages') }}">
                @error('pages')
                ...
                <select id="format_fk" name="format_fk" class="form-control" @error('format_fk') aria-describedby="error-format_fk" @enderror>
                    ...
                    @foreach($formats as $format)
                        <option 
                            ...
                            @selected(old('format_fk') == $format->format_id )
                        >
                            {{ $format->name }}
                        </option>
                    @endforeach
            ...
            <div class="mb-3">
                ...
                <input type="text" id="price" name="price" class="form-control" @error('price') aria-describedby="error-price" @enderror value="{{ old('price') }}">
                @error('price')
            ...
            <fieldset class="mb-3">
                ...
                <label class="me-2">
                    <input
                        ...
                        @checked(in_array($genre->genre_id, old('genre_fk', [])))
                    >
                    {{ $genre->name }}
                </label>
                @endforeach
